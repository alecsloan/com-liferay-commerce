...
public class CommerceRecipientDefinitionTermContributor
	...
	private String _getUserIds(
			CommerceAccount commerceAccount, Role accountAdminRole)
		throws PortalException {
		...
		StringBundler resultsSB = new StringBundler(1);
		...
		for (CommerceAccountUserRel commerceAccountUserRel :
			...
			List<Role> userRoles = _roleLocalService.getUserRoles(
				user.getUserId());

			if (userRoles.contains(accountAdminRole)) {
				resultsSB.append(user.getUserId());
				resultsSB.append(",");
			}
		}

		resultsSB.setIndex(resultsSB.index() - 1);
